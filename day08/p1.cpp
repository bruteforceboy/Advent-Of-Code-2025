...
auto solve(T vec) -> void {
    ...
    std::set<int> comps;
    std::vector<int> comp_vals;
    for (int i = 0; i < n; i++)
        if (!comps.count(dsu.findSet(i)))
            comp_vals.push_back(dsu.getSize(dsu.findSet(i))),
                comps.emplace(dsu.findSet(i));

    std::ranges::sort(comp_vals, std::greater<int>());
    std::cout << comp_vals[0] * comp_vals[1] * comp_vals[2] << "\n";
}
